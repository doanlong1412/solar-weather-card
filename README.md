...
### 📊 Biểu đồ dự báo sản lượng Solar *(Mới v1.6)*
...
- Có thể **bật/tắt hoàn toàn** trong Visual Editor (`show_forecast_chart`) — ngay dưới Node glow effect

### 🎨 Tùy chỉnh màu sắc *(Mới v1.6)*
...
| `show_forecast_chart` | `true` / `false` | `true` | Bật/tắt biểu đồ dự báo sản lượng solar *(Mới v1.6)* |
| `currency` | ký hiệu bất kỳ | `đ` | Đơn vị tiền tệ |
| `pricing_tiers` | xem bên dưới | EVN VNĐ | Thang giá điện tùy chỉnh |

---

### Tùy chọn màu sắc *(Mới v1.6)*
...
## 📊 Hướng dẫn cài đặt Biểu đồ dự báo Solar *(Mới v1.6)*
...
Nếu chưa cài Solcast, bạn tạo sensor tính toán dựa trên vị trí mặt trời và độ che phủ mây để làm đường dự báo fallback.

#### Bước 1 — Tạo `input_text` để lưu dữ liệu thực tế theo giờ

**Cách A — Tạo qua UI (dễ nhất):**

Vào **Settings → Devices & Services → Helpers → Add Helper → Text** và đặt tên `solar_live_curve`.
...
#### Bước 2 — Tạo sensor dự báo và automation ghi thực tế
...
automation:
  - alias: "Solar Live Curve - Ghi thực tế mỗi giờ"
    trigger:
...
> ⚠️ **Thay thế trước khi dùng:**
> - `weather.YOUR_WEATHER_ENTITY` → entity thời tiết của bạn (vd: `weather.forecast_home`)
> - `sensor.YOUR_SOLAR_ENTITY` → sensor công suất solar thực (vd: `sensor.lux_solar_output_live`)
> - `8000` → công suất peak của hệ thống bạn tính bằng W (vd: hệ 10kWp → `10000`)

---

#### Bước 3 — Thêm entity vào card

```yaml
solcast_detail_entity: sensor.solar_forecast_now
```

Card tự nhận dạng đây là sensor tính toán và vẽ đường dự báo phù hợp.
...
- 🎛️ **Toggle ẩn/hiện biểu đồ** — `show_forecast_chart` trong Visual Editor (ngay dưới Node glow effect)
