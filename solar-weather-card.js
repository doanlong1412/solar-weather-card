...
class SolarWeatherCardEditor extends HTMLElement {
  ...
  static get FIELDS() {
    return [
      ...
      { key: 'solar_pv1_voltage_entity', label: '⚡ Điện áp PV Array 1 (V)',     required: false },
      { key: 'solar_pv2_voltage_entity', label: '⚡ Điện áp PV Array 2 (V)',     required: false },
  ...
  _render() {
    const flowStyle = this._config.flow_style || 'line';
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .flow-row{display:flex;flex-direction:column;margin-bottom:16px}
        .flow-row label{font-size:13px;font-weight:600;color:var(--primary-text-color);margin-bottom:8px}
        .flow-btns{display:flex;gap:8px}
        .flow-btn{flex:1;padding:10px 8px;border-radius:8px;border:1.5px solid var(--divider-color);
        ...
        .flow-btn.active{border-color:var(--primary-color);background:rgba(3,169,244,.12);
          color:var(--primary-color);font-weight:700}
        .divider{height:1px;background:var(--divider-color);margin:12px 0 16px}
      ...
      <div class="flow-row">
        <label>🌊 Kiểu hiển thị luồng điện</label>
        <div class="flow-btns">
          <div class="flow-btn ${flowStyle === 'line' ? 'active' : ''}" data-flow="line">
            ── Đường kẻ động
          </div>
          <div class="flow-btn ${flowStyle === 'particle' ? 'active' : ''}" data-flow="particle">
            ✦ Bong bóng hạt
          </div>
        </div>
      </div>
    ...
    // Flow style buttons
    this.shadowRoot.querySelectorAll('.flow-btn').forEach(btn => {
      btn.addEventListener('click', e => {
        const val = e.currentTarget.dataset.flow;
        this._config = { ...this._config, flow_style: val };
        this._fire();
        this._render();
      });
    });
...
function makeWeatherIcon(ws) {
  ...
  if (ws==='rainy') {
    ...
    return `${css}<div class="wi">
      <div class="cl cl-g cl-abs" style="top:8px;width:44px;height:20px;"></div>
  ...
  if (ws==='pouring') {
    ...
    return `${css}<div class="wi">
      <div class="cl cl-dk cl-abs" style="top:5px;width:46px;height:20px;"></div>
      <div class="dps" style="gap:4px;bottom:2px;">${hdrps}</div></div>`;
  }
  if (ws==='lightning') return `${css}<div class="wi">
    <div class="cl cl-dk cl-abs" style="top:4px;width:46px;height:20px;"></div>
...
class SolarWeatherCard extends HTMLElement {
  ...
  // ── PARTICLE flow (bong bóng) ─────────────────────────────────
  _particleLevel(w) {
    if (w < 1000)      return { dur: 1.8,  count: 7,  size: 2.8 };
    else if (w < 2000) return { dur: 1.1,  count: 11, size: 3.2 };
    else if (w < 4000) return { dur: 0.65, count: 17, size: 3.8 };
    else               return { dur: 0.28, count: 24, size: 4.2 };
  }
  _particleFlow(pathD, pathId, color, glowColor, fl) {
    let html = '';
    html += `<path d="${pathD}" fill="none" stroke="${color}" stroke-width="1.2" stroke-dasharray="3,14" opacity="0.18" stroke-linecap="round"/>`;
    // Main particles
    for (let i = 0; i < fl.count; i++) {
      const delay = (i / fl.count * fl.dur).toFixed(3);
      const sz    = (fl.size * (0.55 + Math.random()*0.9)).toFixed(2);
      const op    = (0.55 + Math.random()*0.45).toFixed(2);
      html += `<circle r="${sz}" fill="${color}" opacity="${op}">
        <animateMotion dur="${fl.dur}s" begin="-${delay}s" repeatCount="indefinite" rotate="auto">
          <mpath href="#${pathId}"/>
        </animateMotion>
      </circle>`;
    }
    // Glow particles
    const glowCount = Math.ceil(fl.count / 3);
    const glowDur   = (fl.dur * 1.5).toFixed(2);
    for (let j = 0; j < glowCount; j++) {
      const gdelay = (j / glowCount * fl.dur * 1.5).toFixed(3);
      const gsz    = (fl.size * 2.4).toFixed(2);
      html += `<circle r="${gsz}" fill="${glowColor}" opacity="0.20" filter="url(#sf)">
        <animateMotion dur="${glowDur}s" begin="-${gdelay}s" repeatCount="indefinite" rotate="auto">
          <mpath href="#${pathId}"/>
        </animateMotion>
      </circle>`;
    }
    // Sparkle
    const sparkCount = Math.ceil(fl.count * 0.6);
    for (let k = 0; k < sparkCount; k++) {
      const sdelay = (Math.random() * fl.dur).toFixed(3);
      const sdur   = (fl.dur * (0.6 + Math.random()*0.8)).toFixed(2);
      const offX   = (Math.random()*16 - 8).toFixed(1);
      const offY   = (Math.random()*16 - 8).toFixed(1);
      const ssz    = (1.0 + Math.random()*2.0).toFixed(2);
      const sop    = (0.25 + Math.random()*0.5).toFixed(2);
      html += `<g transform="translate(${offX},${offY})">
        <circle r="${ssz}" fill="${color}" opacity="${sop}">
          <animateMotion dur="${sdur}s" begin="-${sdelay}s" repeatCount="indefinite">
            <mpath href="#${pathId}"/>
          </animateMotion>
        </circle>
      </g>`;
  ...
  _render() {
    ...
    if (!Object.values(this._config).some(v => v && String(v).trim() && v !== 'line' && v !== 'particle')) {
    ...
    // ── Layout constants (khác nhau giữa 2 mode) ──────────────
    let BAT_X, BAT_Y, BAT_W, BAT_H;
    let GRD_X, GRD_Y, GRD_W, GRD_H;
    let INV_X, INV_Y, INV_W, INV_H;
    let HOM_X, HOM_Y, HOM_W, HOM_H;
    let svgHeight;

    if (isParticle) {
      BAT_X=-40; BAT_Y=140; BAT_W=90;  BAT_H=120;
      GRD_X=296;  GRD_Y=148; GRD_W=90;  GRD_H=120;
      INV_X=128;  INV_Y=268; INV_W=90;  INV_H=110;
      HOM_X=128;  HOM_Y=428; HOM_W=90;  HOM_H=100;
      svgHeight = 560;
    } else {
      BAT_X=0;   BAT_Y=148; BAT_W=90;  BAT_H=90;
      GRD_X=256; GRD_Y=148; GRD_W=90;  GRD_H=90;
      INV_X=128; INV_Y=268; INV_W=90;  INV_H=90;
      HOM_X=128; HOM_Y=408; HOM_W=90;  HOM_H=82;
      svgHeight = 520;
    }

    const BAT_CX=BAT_X+BAT_W/2, BAT_CY=BAT_Y+BAT_H/2;
    const BAT_R=BAT_X+BAT_W,    BAT_BOT=BAT_Y+BAT_H;
    const GRD_CX=GRD_X+GRD_W/2, GRD_CY=GRD_Y+GRD_H/2, GRD_L=GRD_X;
    const INV_CX=INV_X+INV_W/2, INV_CY=INV_Y+INV_H/2;
    const INV_L=INV_X, INV_R=INV_X+INV_W, INV_TOP=INV_Y, INV_BOT=INV_Y+INV_H;
    const HOM_CX=HOM_X+HOM_W/2, HOM_TOP=HOM_Y;

    // ── Build flow lines / particles ──────────────────────────
    let FL = '';
    let fpDefs = ''; // chỉ dùng cho particle mode
    let fpIdx = 0;

    if (isParticle) {
      // Helper particle
      const addParticle = (pathD, color, glow, w) => {
        const id = `fp-${fpIdx++}`;
        fpDefs += `<path id="${id}" d="${pathD}"/>`;
        FL += this._particleFlow(pathD, id, color, glow, this._particleLevel(w));
      };

      if (hasSolar) {
        addParticle(
          `M ${bx.toFixed(1)},${(by+7).toFixed(1)} C ${bx.toFixed(1)},${INV_TOP-70} ${INV_CX},${INV_TOP-150} ${INV_CX},${INV_TOP}`,
          'rgba(255,235,80,.95)', 'rgba(255,200,50,.7)', solarW);
      }
      if (isCharge) {
        addParticle(
          `M${INV_L+30},${INV_BOT} C ${INV_L},${INV_BOT+90} ${BAT_CX},${BAT_BOT+180} ${BAT_CX},${BAT_BOT}`,
          'rgba(80,235,130,.95)', 'rgba(40,200,90,.6)', battFlW);
      ...
      if (isDisch) {
        addParticle(
          `M${BAT_R},${BAT_CY} C ${BAT_R},${BAT_CY+40} ${INV_CX},${INV_TOP-90} ${INV_CX-10},${INV_TOP}`,
          'rgba(255,210,60,.95)', 'rgba(200,150,20,.6)', Math.abs(battFlW));
      ...
      if (hasGrid) {
        const gp = gridFlW > 10
          ? `M${INV_CX},${INV_TOP} C ${INV_CX},${INV_TOP-60} ${GRD_L},${GRD_CY+60} ${GRD_L},${GRD_CY}`
          : `M${GRD_L},${GRD_CY} C ${GRD_L},${GRD_CY+40} ${INV_CX+10},${INV_TOP-90} ${INV_CX+10},${INV_TOP}`;
        addParticle(gp, 'rgba(100,190,255,.95)', 'rgba(50,140,230,.6)', Math.abs(gridFlW));
      ...
      if (hasHome) {
        addParticle(
          `M${INV_CX},${INV_BOT+2} C ${INV_CX},${INV_BOT+20} ${HOM_CX},${HOM_TOP-20} ${HOM_CX},${HOM_TOP-2}`,
          'rgba(255,160,60,.95)', 'rgba(200,100,20,.6)', homeFlW);
      ...
      if (hasHome) {
        const p=`M${INV_CX},${INV_BOT+2} L${HOM_CX},${HOM_TOP-2}`;
    ...
    // ── Node icons ────────────────────────────────────────────
    const bx0=BAT_CX-26, ty0=BAT_Y+10;
    const fw=Math.max(2,Math.round(44*battSoc/100));
    const fc=battSoc>50?'rgba(80,225,120,.95)':battSoc>20?'rgba(255,200,55,.95)':'rgba(255,80,80,.95)';
    const battIco=`
      <rect x="${bx0}" y="${ty0}" width="52" height="24" rx="5" fill="rgba(255,255,255,.07)" stroke="rgba(120,240,150,.75)" stroke-width="1.6"/>
      <rect x="${bx0+52}" y="${ty0+7}" width="5" height="10" rx="2.5" fill="rgba(120,240,150,.55)"/>
      <rect x="${bx0+4}" y="${ty0+4}" width="${fw}" height="16" rx="2" fill="${fc}"/>
      <text x="${BAT_CX}" y="${ty0+16}" text-anchor="middle" fill="rgba(255,255,255,.95)" font-size="9" font-weight="700" font-family="Inter">${bPct}%</text>`;

    const cx2=INV_CX, ty2=INV_Y+10;
    const invIco=`
    ...
    // ── Inverter text (particle mode có thêm pvDC) ──────────
    const invTextExtra = isParticle
      ? `<text x="${INV_CX}" y="${INV_Y+83}" text-anchor="middle" fill="rgba(100,240,155,.95)" font-size="10" font-family="Inter">${bVolt}V BAT</text>
         <text x="${INV_CX}" y="${INV_Y+96}" text-anchor="middle" fill="rgba(255,220,80,.95)" font-size="10" font-family="Inter">${pvDC}V PV</text>`
    ...
    this.shadowRoot.innerHTML = `
      ...
      <div style="font-family:Inter,-apple-system,sans-serif;color:#fff;
        ...
        backdrop-filter:blur(${isParticle?'5':'24'}px);-webkit-backdrop-filter:blur(${isParticle?'5':'24'}px);
        ...
        <div style="padding:18px 16px 14px;border-bottom:1px solid rgba(255,255,255,.12);background:rgba(255,255,255,.04);">
          ...
          <div style="display:flex;align-items:center;gap:8px;background:rgba(255,255,255,.07);border:1px solid rgba(255,255,255,.12);border-radius:12px;padding:${isParticle?'7px':'1px'} 11px;font-size:${isParticle?'11.5':'12'}px;">
            <span style="color:rgba(255,255,255,.45);">Ngày mai</span>
            <div style="transform:scale(${isParticle?'0.55':'0.5'});transform-origin:left center;width:40px;height:40px;flex-shrink:0;">${tmrIconHTML}</div>
        ...
        <div style="padding:8px 0 10px;border-bottom:1px solid rgba(255,255,255,.10);">
          ...
          <div style="position:relative;width:100%;height:${isParticle?'500':'520'}px;">
            <svg viewBox="0 0 346 ${svgHeight}" xmlns="http://www.w3.org/2000/svg" style="position:absolute;inset:0;width:100%;height:100%;overflow:visible;">
              <defs>
              ...
              <!-- BATTERY -->
              <rect x="${BAT_X}" y="${BAT_Y}" width="${BAT_W}" height="${BAT_H}" rx="16" fill="url(#battG)" stroke="rgba(80,220,120,.45)" stroke-width="1.5"/>
              <rect x="${BAT_X}" y="${BAT_Y}" width="${BAT_W}" height="${BAT_H}" rx="16" fill="rgba(255,255,255,.07)"/>
              ${battIco}
              <text x="${BAT_CX}" y="${BAT_Y+(isParticle?62:56)}" text-anchor="middle" fill="#fff" font-size="19" font-weight="700" font-family="Inter" filter="url(#tf)">${battW}W</text>
              <text x="${BAT_CX}" y="${BAT_Y+(isParticle?78:70)}" text-anchor="middle" fill="rgba(100,240,155,.95)" font-size="13" font-family="Inter">${battDir}</text>
              <text x="${BAT_CX}" y="${BAT_Y+(isParticle?94:82)}" text-anchor="middle" fill="rgba(255,255,255,.55)" font-size="12" font-family="Inter">${bVolt}V</text>

              <!-- GRID -->
              <rect x="${GRD_X}" y="${GRD_Y}" width="${GRD_W}" height="${GRD_H}" rx="16" fill="url(#gridG)" stroke="rgba(90,170,255,${hasGrid?'.45':'.18'})" stroke-width="1.5"/>
              <rect x="${GRD_X}" y="${GRD_Y}" width="${GRD_W}" height="${GRD_H}" rx="16" fill="rgba(255,255,255,${hasGrid?'.07':'.02'})"/>
              ${gridIco}
              <text x="${GRD_CX}" y="${GRD_Y+(isParticle?68:56)}" text-anchor="middle" fill="${hasGrid?'#fff':'rgba(255,255,255,.35)'}" font-size="17" font-weight="700" font-family="Inter" filter="url(#tf)">${gridW}W</text>
              <text x="${GRD_CX}" y="${GRD_Y+(isParticle?82:70)}" text-anchor="middle" fill="rgba(140,200,255,${hasGrid?'.95':'.4'})" font-size="13" font-family="Inter">${gridDir}</text>
              <text x="${GRD_CX}" y="${GRD_Y+(isParticle?100:82)}" text-anchor="middle" fill="rgba(255,255,255,.55)" font-size="12" font-family="Inter">${gridV}V AC</text>

              <!-- INVERTER -->
              <rect x="${INV_X}" y="${INV_Y}" width="${INV_W}" height="${INV_H}" rx="16" fill="url(#invG)" stroke="rgba(160,185,255,.45)" stroke-width="1.5"/>
              <rect x="${INV_X}" y="${INV_Y}" width="${INV_W}" height="${INV_H}" rx="16" fill="rgba(255,255,255,.07)"/>
              ${invIco}
              <text x="${INV_CX}" y="${INV_Y+(isParticle?55:50)}" text-anchor="middle" fill="rgba(255,255,255,.6)" font-size="7.5" font-weight="700" font-family="Inter" letter-spacing="1.5">INVERTER</text>
              <text x="${INV_CX}" y="${INV_Y+(isParticle?70:64)}" text-anchor="middle" fill="rgba(160,215,255,.98)" font-size="${isParticle?11:13}" font-family="Inter" filter="url(#tf)">${gridV}V AC</text>
              ${invTextExtra}

              <!-- HOME -->
              <rect x="${HOM_X}" y="${HOM_Y}" width="${HOM_W}" height="${HOM_H}" rx="16" fill="url(#homeG)" stroke="rgba(255,155,65,${hasHome?'.45':'.18'})" stroke-width="1.5"/>
              <rect x="${HOM_X}" y="${HOM_Y}" width="${HOM_W}" height="${HOM_H}" rx="16" fill="rgba(255,255,255,${hasHome?'.07':'.02'})"/>
              ${homeIco}
              <text x="${HOM_CX}" y="${HOM_Y+(isParticle?68:65)}" text-anchor="middle" fill="${hasHome?'#fff':'rgba(255,255,255,.35)'}" font-size="19" font-weight="700" font-family="Inter" filter="url(#tf)">${homeW}W</text>
              <text x="${HOM_CX}" y="${HOM_Y+(isParticle?83:78)}" text-anchor="middle" fill="rgba(255,185,95,${hasHome?'.95':'.4'})" font-size="12" font-family="Inter">Nhà tiêu thụ</text>
            </svg>
          </div>
        </div>
...
window.customCards.push({
  ...
  description: 'Solar + Battery + Weather — Line flow hoặc Particle bubble, cấu hình entity qua UI',
  preview:     false,
